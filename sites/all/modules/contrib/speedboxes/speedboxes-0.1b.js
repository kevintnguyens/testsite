/**
 *  Copyright (c) 2012 Bright Solutions GmbH
 *  
 *  Speedboxes allows you to select multiple checkboxes on the page and check/uncheck/reverse ticks with a single click.
 *  By using a bookmarklet you can use this script on any page and simply toggle it on or off allowing you to still select text etc.
 *  This script requires jQuery 1.4.3 or later. If no jQuery script was loaded yet, the latest version will be loaded automatically.
 *  
 *  @visit http://www.brightsolutions.de/
...
window.loadSpeedboxes = function( $ ) {
  ...
  $( function() {
    ...
    function updateEditingPopup() {
      ...
      if( pSelectedCheckboxes.not(':checked').size() )
      ...
      if( pSelectedCheckboxes.filter(':checked').size() )
